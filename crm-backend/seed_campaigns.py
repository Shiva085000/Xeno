...
def seed_campaigns():
    """Idempotent: ensures each demo campaign exists by name. Never deletes
    anything, so user/agent-created campaigns are preserved and a single
    leftover row can no longer block demo seeding."""
    Base.metadata.create_all(bind=engine)
    db = SessionLocal()
    try:
        created = 0
        for spec in DEMO_CAMPAIGNS:
            exists = db.query(Campaign).filter(Campaign.name == spec["name"]).first()
            if exists:
                continue
            _seed_campaign(db, **spec)
            created += 1
        print(f"Demo campaigns ensured ({created} created).")
    finally:
        db.close()
